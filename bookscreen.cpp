...
static void gotoVNScreenCB(void*);
...
public:
	...
	BookScreen()
	{
		...
		loadScreenEntities();
		setTextActive();
		streamMusicFile("music/title.ogg");
	}

	void loadBookTexts()
	{
		MugenDefScript script;
		loadMugenDefScript(&script, "book/BOOK.def");
	...
	void loadFiles()
	{
		mSprites = loadMugenSpriteFileWithoutPalette(std::string("book/") + gBookScreenData.mBookName +".sff");
		mAnimations = loadMugenAnimationFile(std::string("book/") + gBookScreenData.mBookName + ".air");
		mSounds = loadMugenSoundFile("book/BOOK.snd");
	...
	void loadScreenEntities()
	{
		...
		mTextId = addMugenTextMugenStyle(" ", Vector3D(40, 200, 3), Vector3DI(2, 7, 1));
...
static void gotoVNScreenCB(void*)
{
	if (gBookScreenData.mBookName == "final2")
	{
		setBookName("intro");
